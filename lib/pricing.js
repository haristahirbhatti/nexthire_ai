...
export async function updateProductPrice(productId, newAmount) {
  ...
  if (supabase) {
    ...
    if (error) {
      console.error("[pricing] Failed to save updated price to Supabase:", error);
      throw new Error(`Database error: ${error.message}. Price was updated in temporary memory only.`);
    }
