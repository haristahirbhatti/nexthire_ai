...
import { getTemplatePage } from "@/data/templates";
...
function resolveTemplate(templateId) {
  ...
  // getTemplatePage returns items starting at that index page (1 per page = exact item)
  const pageNum = id;
  const items = getTemplatePage(pageNum, 1);
  const t = items[0];
  ...
  return {
    accent: t.accent,
    font: fontMap[t.tone] || "Calibri, Arial, sans-serif",
    columns: t.columns,
    tone: t.tone,
    name: t.name,
  };
