...
export function downloadWordDocument(pkg, templateId) {
  ...
  const skillsByCat = pkg?.skillsByCategory        || {};
  ...
  const sidebarWordHTML = isTwo ? `
    <td valign="top" style="width:32%; background-color:${sidebarBgColor}; padding:16pt 12pt; color:#ffffff; border-radius:4pt;">
      ...
      ${certs.length ? `
      <p style="font-size:8.5pt; font-weight:bold; text-transform:uppercase; color:${tmpl.accent}; margin:14pt 0 6pt 0; border-bottom:1pt solid ${tmpl.accent}; padding-bottom:2pt; letter-spacing:0.5px;">Certifications</p>
      ${certs.map(c => `
        <p style="font-size:8pt; color:#e2e8f0; margin:0 0 3pt 0;">🏆 <strong>${c.name}</strong>${c.issuer ? ` (${c.issuer})` : ""}</p>
      `).join("")}
      ` : ""}

      ${languages.length ? `
      <p style="font-size:8.5pt; font-weight:bold; text-transform:uppercase; color:${tmpl.accent}; margin:14pt 0 6pt 0; border-bottom:1pt solid ${tmpl.accent}; padding-bottom:2pt; letter-spacing:0.5px;">Languages</p>
      ${languages.map(l => `
        <p style="font-size:8pt; color:#e2e8f0; margin:0 0 3pt 0;">🌐 ${l.language} ${l.proficiency ? `(${l.proficiency})` : ""}</p>
      `).join("")}
      ` : ""}
    </td>
  ...
  const projectsWordHTML = projects.length ? `
    <h2>Key Projects &amp; Technical Highlights</h2>
    ${projects.map(p => `
  ...
  const htmlContent = `
    ...
    <body>
      ...
      ${!isTwo ? `
      <h2>Core Competencies &amp; Key Skills</h2>
      ...
      ${education.map(edu => `
        <p style="margin:0 0 3pt 0;"><strong style="color:#0f172a;">${edu.degree}</strong> &mdash; <span style="color:#475569;">${edu.institution}</span> (${edu.year}) ${edu.details ? `<span style="font-size:8.5pt; color:#64748b;">&bull; ${edu.details}</span>` : ""}</p>
      `).join("")}
      ` : ""}
      ...
      ${certs.map(c => `
        <p style="margin:0 0 3pt 0;">🏆 <strong style="color:#0f172a;">${c.name}</strong>${c.issuer ? ` &mdash; <span style="color:#475569;">${c.issuer}</span>` : ""} ${c.year ? `(${c.year})` : ""}</p>
      `).join("")}
      ` : ""}

      ${!isTwo && languages.length ? `
      <h2>Languages</h2>
  ...
  const twoColBodyHTML = `
    <div style="display:grid; grid-template-columns:32% 68%; min-height:850px; background:#fff; border-radius:6px; overflow:hidden;">
      ...
      <div style="background:${sidebarBgColor}; padding:24px 18px; color:#fff;">
        ...
        ${certs.length ? `
        <div style="font-size:8pt; font-weight:800; text-transform:uppercase; color:${tmpl.accent}; border-bottom:1.5px solid ${tmpl.accent}; padding-bottom:3px; margin-bottom:8px; letter-spacing:0.8px;">Certifications</div>
        <div style="display:flex; flex-direction:column; gap:6px; margin-bottom:16px;">
          ${certs.map(c => `
            <div style="font-size:8pt; color:#e2e8f0;">🏆 <strong>${c.name}</strong></div>
          `).join("")}
        </div>
        ` : ""}

        ${languages.length ? `
        <div style="font-size:8pt; font-weight:800; text-transform:uppercase; color:${tmpl.accent}; border-bottom:1.5px solid ${tmpl.accent}; padding-bottom:3px; margin-bottom:8px; letter-spacing:0.8px;">Languages</div>
        <div style="display:flex; flex-direction:column; gap:4px;">
          ${languages.map(l => `
            <div style="font-size:8pt; color:#cbd5e1;">🌐 ${l.language} ${l.proficiency ? `(${l.proficiency})` : ""}</div>
          `).join("")}
        </div>
        ` : ""}
      </div>
  ...
  const singleColBodyHTML = `
    <div style="padding:16px 20px;">
      ...
      <div style="display:flex; flex-direction:column; gap:6px; margin-bottom:14px;">
        ${education.map(edu => `
          <div style="font-size:9.5pt;"><strong style="color:#0f172a;">${edu.degree}</strong> &mdash; <span style="color:#475569;">${edu.institution}</span> <span style="color:#64748b; font-size:8.5pt;">(${edu.year})</span> ${edu.details ? `<span style="color:#64748b; font-size:8.5pt;">&bull; ${edu.details}</span>` : ""}</div>
        `).join("")}
      </div>
      ` : ""}
      ...
      <div style="display:flex; flex-direction:column; gap:4px; margin-bottom:14px;">
        ${certs.map(c => `
          <div style="font-size:9pt;">🏆 <strong style="color:#0f172a;">${c.name}</strong>${c.issuer ? ` &mdash; <span style="color:#475569;">${c.issuer}</span>` : ""} ${c.year ? `<span style="color:#64748b;">(${c.year})</span>` : ""}</div>
        `).join("")}
      </div>
      ` : ""}
      ...
      <div style="display:flex; gap:12px; flex-wrap:wrap;">
        ${languages.map(l => `
          <span style="font-size:9pt; color:#334155;"><strong>${l.language}</strong>${l.proficiency ? ` (${l.proficiency})` : ""}</span>
        `).join("")}
      </div>
      ` : ""}
    </div>
